...
export const PDFBarcodeJs = (function () {
  var env = "PROD";
  ...
  function arrangeConfigs(settings) {
    if (
      !settings.resultOpts.multiCodesInPage ||
      settings.resultOpts.singleCodeInPage
    ) {
      settings.resultOpts.maxCodesInPage = 1; // then it should be single
      settings.quagga.decoder.multiple = false;
    } else settings.quagga.decoder.multiple = true; // if not single then set multi so quagga can decode multiple at one time
    ...
    if (settings.scale.once) {
      ...
      for (
        i = settings.scale.start;
        i <= settings.scale.stop;
        i += settings.scale.step
      ) {
  ...
  function getPageAsImg(canvas, scaled, noisify) {
    ...
    if (scaled === 1) {
      const imgToBePrinted = createCanvas(canvas.width, canvas.height);
      imgToBePrinted.getContext("2d").putImageData(imageData, 0, 0);
      result.printImg = imgToBePrinted;
    }

    var imgToProcess = createCanvas(canvas.width, canvas.height);
    var context2 = imgToProcess.getContext("2d");
    context2.putImageData(imageData, 0, 0);

    if (noisify) {
      for (var y = 0; y < canvas.height; y += 60) {
        //todo 60 worked good, but should be related to img sizee
  ...
  function setResult(
    ...
    if (settings.resultOpts.singleCodeInPage) {
      // when single result  quagga returns obj only
        ...
        if (env === "DEV")
          console.log(
            "PAGE_" +
              ...
              " times with " +
              patch +
              " patch has no results."
          );
      }
    } // when multi result obj of quagga returns array
    else {
      var hasResult = false;
      if (result) {
        for (var i = 0; i < result.length; i++) {
          if (result[i].codeResult) {
            ...
            var pageFormatCollector =
              document_barcode.codesByPageAndFormat[currentPage][
                codeResult.format
              ];
            if (
              pageFormatCollector !== undefined &&
              pageFormatCollector.indexOf(codeResult.code) !== -1
            )
              continue;

            if (
              document_barcode.codesByPage[currentPage].length ===
              settings.resultOpts.maxCodesInPage
            )
              // page limit reached
              break;
  ...
  function addResult(codeResult, resultObj, page, patch, scale) {
    if (env === "DEV") console.log("Adding " + codeResult.code);
    ...
    if (resultObj.codesByPageAndFormat[page][codeResult.format] === undefined)
      // this array helps to prevent same barcoodes in a page with same format to be collected
      resultObj.codesByPageAndFormat[page][codeResult.format] = [
        codeResult.code,
      ];
    else
      resultObj.codesByPageAndFormat[page][codeResult.format].push(
        codeResult.code
      );

    if (resultObj.codesByFormat[codeResult.format] === undefined)
      // this arr
      resultObj.codesByFormat[codeResult.format] = [codeResult.code];
    else resultObj.codesByFormat[codeResult.format].push(codeResult.code);
  ...
  function shuffle2Optimize(arr: any[], first: any, type: any) {
    if (env === "DEV") {
    ...
    while (ordered.length !== myarrlen) {
      left--;
      right++;
      if (left >= 0) ordered.push(arr[left]);
      if (right < myarrlen) ordered.push(arr[right]);
    }
  ...
  const copyobj = (source: any, deep: any) => {
    var o, prop, type;
    ...
    for (prop in source) {
      if (source.hasOwnProperty(prop)) {
        ...
        if (deep && type == "object" && source[prop] !== null) {
          o[prop] = copyobj(source[prop], null);
        } else {
          o[prop] = source[prop];
        }
      }
  ...
  function decoder(params) {
    ...
    var currentPage = 1;
    var quagaconfigs = copyobj(settings.quagga, null);

    const pdf = params.pdfDocument;
    if (params.singlePage) {
      if (!pageInRange(pdf, params.pageNr)) {
        params.final_call_back({
          success: false,
          message:
            "Given page is out of range! Valid page range is (1-" +
            pdf.numPages +
            ")",
        });
        return false;
    ...
    function getPage() {
      if (env === "DEV") console.log("PAGE_" + currentPage + " fetched...");
      pdf.getPage(currentPage).then(async function (page) {
        if (env === "DEV") console.log("PAGE_" + currentPage + " rendered...");
        ...
        var postPdfJsPageRender = function (canvas, scaled, scalingTime) {
          return new Promise((resolve) => {
            ...
            var postQuaggaDecode = function (result) {
              ...
              if (
                (finalResults.codesByPage[currentPage] as any).length <
                settings.resultOpts.maxCodesInPage
              ) {
                patch_attempts++;
        ...
        for (var i = 0; i < scale_len; i++) {
          ...
          if (env === "DEV")
            console.log(
              "PAGE_" + currentPage + " scaling " + scaled + " times."
            );

          var result: any = await page
            .render({
              canvasContext: canvas.getContext("2d"),
              viewport: viewport,
            })
            .promise.then(() =>
              postPdfJsPageRender(canvas, scaled, scalingTime)
            );
          ...
          if (result.isDone) {
            if (env === "DEV")
              console.log(
                "PAGE_ " +
                  currentPage +
                  " was finished! Stopping the loop of scale! (on scale " +
                  scaled +
                  ")"
              );

            if (settings.improve) {
              // for the next page it will start with this patch and scale
              settings.patches = shuffle2Optimize(
                settings.patches,
                result.patch,
                "Patch"
              );
              settings.scale.ordered = shuffle2Optimize(
                settings.scale.ordered,
                scaled,
                "Scale"
              );
        ...
        if (!params.singlePage && currentPage < pdf.numPages) {
          currentPage++;
          getPage();
        } else params.final_call_back(parseResult(finalResults, params));
      });
    }
    return false;
  }
  ...
  function getValidPatches(img, settings) {
    function checkPatchValidity(img, patchSize, half) {
      function _computeDivisors(n: number) {
        var largeDivisors: any[] = [],
          divisors: any[] = [],
          i: number = -1;
      ...
      var size = {};
      if (half) size = { x: img.width / 2, y: img.height / 2 };
      else size = { x: img.width, y: img.height };

      var calculated_patch_size = calculatePatchSize(patchSize, size);
      if (
        calculated_patch_size &&
        (calculated_patch_size.x * calculated_patch_size.y * 3 > 65536 ||
          calculated_patch_size.x * calculated_patch_size.y * 3 +
            calculated_patch_size.x * calculated_patch_size.y >
            65536)
      )
        return false;
      return true;
    ...
    for (var i = 0; i < patches_len; i++) {
      if (
        checkPatchValidity(
          img,
          settings.patches[i],
          settings.quagga.locator.halfSample
        )
      )
